...
$(".vbtn").click(function(){
    var  vname=$(this).parent("form").find($(".lyname"));
    var  vtel=$(this).parent("form").find($(".lytel"));
    var  vmail=$(this).parent("form").find($(".lymail"));
    var  vpro=$(this).parent("form").find($(".lypro"));
    var  vcom=$(this).parent("form").find($(".lycom"));
    var  vcontect=$(this).parent("form").find($(".lytxt"));
    checkwrint1(vname);
    checkwrint2(vtel);
    checkwrint3(vmail);
    checkwrint4(vcom);
    checkwrint5(vcontect);
    checkwrint6(vpro);
    
    if (vname.hasClass("no")||vtel.hasClass("no")||vmail.hasClass("no")||vpro.hasClass("no")||vcom.hasClass("no")||vcontect.hasClass("no")){
           console.log("over");
    }else {
//写入数据库
        $.post("ajax/getdemo.asp", {
...
$(".lyname").blur(function(){
    checkwrint1(this);
});
$(".lytel").blur(function(){
    checkwrint2(this);
});
$(".lymail").blur(function(){
    checkwrint3(this);
});
...
function checkwrint4(v){
    if ($(v).val()==""||$(v).val()=="邮箱地址"){
        $(v).addClass("no");
        $(v).next(".err").html("× 请输入您的公司名称");//邮箱地址不能为空
    }else{
        $(v).removeClass("no");
        $(v).next(".err").html("");
    }
}
function checkwrint5(v){
    if ($(v).val()==""||$(v).val()=="邮箱地址"){
        $(v).addClass("no");
        $(v).next(".err").html("× 请说明您的需求");//邮箱地址不能为空
    }else{
        $(v).removeClass("no");
        $(v).next(".err").html("");
    }
...
function checkwrint6(v){
    if ($(v).val()==""||$(v).val()=="邮箱地址"){
        $(v).addClass("no");
        $(v).next(".err").html("× 请选择产品");//邮箱地址不能为空
    }else{
        $(v).removeClass("no");
        $(v).next(".err").html("");
    }
