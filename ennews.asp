...
<div class="titleline">
    <div class="container">
    <div class="row">
        <span class="tit">Fresh news</span><span class="txt hidden-xs">Anything you want</span><!--<span class="tbtn tra hand hidden-xs">在线咨询</span>-->
    </div>
    </div>
</div>
<!---广告结束 ---->
...
<!---->
     <%
       ...
       %>
<li><a href="ennewsDetail.asp?nv=<%=rsnew("prod_id")%>"><Img src="<%=rsnew("prod_pic")%>" class="b_c12"></a><div class="info"><a class="h3" href="ennewsDetail.asp?nv=<%=rsnew("prod_id")%>"><%=rsnew("prod_nameen")%></a><br><span class="ntime"><%=formatdatetime(rsnew("prod_date"),2)%></span><p class="txt"><%=rsnew("summanyen")%></p>
<a href="ennewsDetail.asp?nv=<%=rsnew("prod_id")%>" class="nbtn ">Learn More<em class="more-arr tra">&gt;</em></a></div></li>
<%rsnew.movenext
           	i=i+1
               loop
           	end if%>
